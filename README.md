...
2. **tv2.m3u**：单播地址列表（通过msd_lite转换的组播地址，默认回看参数支持ok影视）
...
按大小排序最大的是回看流量，我的是中兴格式，这个只是针对盒子
...
实现 potplayer 或者 mpv 回看，starttime endtime 修改为昨天间隔5小时内的参数：
...
创建代理配置文件：
- 解析器设置为自己的openwrt ip,就是dns,可以设为223.5.5.5
